function out = testbench_FIRConvolver
% TESTBENCH_FIRCONVOLVER Exercise audio plugin class
% to check for violations of plugin constraints and other errors.
%
% OUT = TESTBENCH_FIRCONVOLVER Return the output data from the
...
%
%   codegen testbench_FIRConvolver    % Create the mex function
%   testbench_FIRConvolver_mex        % Run the mex function
%
...
% validateAudioPlugin.
%
% Automatically generated by validateAudioPlugin 20-Nov-2022 17:37:21
...
for sampleRate = sampleRates
    ...
    for i = 1:numel(frameSizes)
        ...
        val = fromNormalizedoutputGain(mod(floor((i-1)./1),3)/2);
        plugin.outputGain = val;
        paramState.outputGain = val;
        checkForTampering(plugin, paramState, sampleRate, ...
            'Setting parameter ''outputGain''');
...
if ~isequal(paramState.outputGain, plugin.outputGain)
    error('ValidateAudioPlugin:ParamChanged', ...
        '%s changed parameter ''outputGain'' from %g to %g.', ...
        cause, paramState.outputGain, plugin.outputGain);
end
...
paramState.outputGain = plugin.outputGain;
paramState.mix = plugin.mix;
end

function val = fromNormalizedoutputGain(normval)
val = -82 + (12--82)*normval;
end
